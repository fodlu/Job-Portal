...
const FindJobPage = () => {
	...
	const [showFilters, setShowFilters] = useState(false); //to show the selected or applied filters
	...
	// both will be saved on the server and the local storage
	...
	//   that user applied on which job will be founnd by the function
	useEffect(() => {
	...
	const renderLogo = (job) => {
		...
		const colorClass = colors[job.company?.length % colors.length] || colors[0]; //these are for the ui colors

		// these rest of these are the UI parts
		return (
			<div
										...
										<div className={s.actionButtons}>
											...
											<div className={s.actionIcons}>
												...
												<button
													...
													aria-label={
														savedJobs.includes(String(job.id)) ? "Unsave job"
														:	"Save job"
													}
