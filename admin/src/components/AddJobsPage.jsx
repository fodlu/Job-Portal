			...
			<div
				...
				<div
					className={`${s.fieldInner} ${
						error ? s.fieldInnerError
						: focused ? s.fieldInnerFocused
						: s.fieldInnerDefault
					}`}>
		...
		<div className={s.uploadContainer}>
			...
			<div
				className={`${s.uploadDropzone} ${
					dragActive ? s.uploadDropzoneActive
					: error ? s.uploadDropzoneError
					: preview ? s.uploadDropzonePreview
	...
	const handleSubmit = async (e) => {
		...
		if (validateForm()) {
			try {
				...
				const response = await API.post(`/job`, formDataToSend, {
					headers: {
				...
				setToast({
					...
					message:
						...
						"Failed to post the job, Try again!",
					type: "error",
				});
			} finally {
				setIsPosting(false);
				...
				<form onSubmit={handleSubmit} className={s.formCard}>
					...
					<div className={s.grid2}>
						<AnimatedField
							...
							onChange={(e) =>
								setFormData({ ...formData, experience: e.target.value || 1 })
							}
						...
						<div className={s.salaryInputWrapper}>
							<div
								className={`${s.salaryInputGroup} ${
									errors.salary ? s.salaryInputGroupError
									: formData.salary.amount ? s.salaryInputGroupFilled
									: s.salaryInputGroupDefault
								}`}>
								<span className={s.salaryIconSpan}>
									<DollarSign
										size={18}
										className={formData.salary.amount ? s.salaryIconFilled : ""}
									/>
								</span>
								<input
									type='number'
