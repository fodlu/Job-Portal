...
const handleSubmit = async (e) => {
    ...
    if(validateForm()) {
        try {
            ...
            } // append all the field filled by the user

            const token = localStorage.getItem("token");

            const response = await axios.post(
                "http://localhost:5000/api/job", formDataToSend, {
                    headers: {
            ...
            setToast({
                show: true,
                message: error?.response?.data?.message || "Failed to post the job, Try again!",
                type: "error"
            })
        } finally {
                setIsPosting(false)
            ...
            <form onSubmit={handleSubmit} className={s.formCard}>
                <div className={s.grid3}>
                    ...
                    <div className={s.mdColSpan2}>
                        <AnimatedField icon={Briefcase} label = "Role Name" name = "roleName" value={formData.roleName} onChange={(e)=> setFormData({...formData, roleName: e.target.value})} error={errors.roleName} placeholder="e.g Senior Frontend Developer" required />
                        <AnimatedField icon={Building2} label = "Company Name" name = "companyName" value={formData.companyName} onChange={(e)=> setFormData({...formData, companyName: e.target.value})} error={errors.companyName} placeholder="e.g Acme Inc" required />
                    </div>
                </div>
                ...
                <div className={s.arraySection}>
                    ...
                    {formData.techStack.map((tech, index) => (
                        <div key={index} className={s.arrayItemRow}>
                            <input type="text" value={tech} onChange={e=> handleArrayChange('techStack', index, e.target.value)} className={`${s.arrayInput} ${errors.techStack && !tech.trim() ? s.arrayInputError : s.arrayInputDefault}`} placeholder='React, Node.js, etc.'/>
                    ...
                    {/* Add more tech stack */}
                    <button type="button" onClick={()=> addArrayField("techStack")} className={s.addBtn}>
                ...
                <div className={s.grid3}>
                    <AnimatedField icon={Briefcase} label = "Job Type" name = "jobType" type='select' value={formData.jobType} onChange={(e)=> setFormData({...formData, jobType: e.target.value})}>
                        <option value="full-time">Full-Time</option>
                        <option value="part-time">Part-Time</option>
                        <option value="contract">Contract</option>
                        <option value="intentship">Intentship</option>
                    </AnimatedField>

                    <AnimatedField icon={Calendar} label = "Post Date (Automatically set to today)" name = "postDate" type='date' value={formData.postDate} onChange={(e)=> setFormData({...formData, postDate: e.target.value})} error={errors.postDate} disabled required>
                        <option value="full-time">Full-Time</option>
                        <option value="part-time">Part-Time</option>
                        <option value="contract">Contract</option>
                        <option value="intentship">Intentship</option>
                    ...
                    <AnimatedField icon={Tag} label = "Category" type='select' name = "category" value={formData.category} onChange={(e)=> setFormData({...formData, category: e.target.value})} error={errors.category} required>
                ...
                {/* submit button */}
                <button type="submit" className={`${s.submitBtn} ${ isPosting ? s.submitBtnDisabled : ""}`} disabled={isPosting}>
                    {isPosting ? (
                        <>
