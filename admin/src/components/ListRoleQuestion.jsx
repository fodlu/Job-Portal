...
// Thid function will return csv file data into plain text
function parseCSV(text) {
	...
	if (nonEmpty.length === 0) return []; // no cell must be empty
...
} // will parse the rolename with data
...
const ListRoleQuestion = () => {
	...
	// to fetch roles coming from the server side
	...
	}, [toasts]); // to remove the preview image
	...
	}; // to upload a new image and create an object to view

	const handleEditChange = (e) => {
		const { name, value } = e.target;
		setEditFormData((prev) => ({ ...prev, [name]: value }));
	}; // handle any form edit
	...
	// to save the edited data to the server
	const handleEditSave = async (id) => {
		try {
			...
			const response = await API.put(`/interview/role/${id}`, formData, {
				headers: {
	...
	//  if we want to cancel the edit or close the edit model
	...
	const handleDeleteConfirm = async () => {
		try {
			const token = localStorage.getItem("token");
			const response = await API.delete(`/interview/role/${deleteRoleId}`, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			});
			...
			if (response.data.success) {
				setRoles((prev) => prev.filter((role) => role._id !== deleteRoleId)); // deleted
