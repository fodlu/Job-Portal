...
const ViewApplicantPage = () => {
    ...
    // this will give you the output as DD/MM/YYYY
                    ...
                    filtered.length === 0 ? (
                        ...
                        <div className={s.grid}>
                            {filtered.map((app, index) => (
                                <div className={s.cardBase} key={app.id} style={{
                                    ...
                                    <div className={s.cardInner}>
                                        ...
                                        <div className={s.detailsWrapper}>
                                            <div className={s.roleRow}>
                                                ...
                                                <span className={s.roleBadge}>{app.appliedForRole}</span>
                                            </div>

                                            <div className={s.dateRow}>
                                                <Calendar className={s.dateIcon} />
                                                <span className={s.dateBadge}>{formatDate(app.appliedAt)}</span>
                                            </div>
                                        </div>

                                        <div className={s.buttonWrapper}>
                                            {app.resumeFile ? (
                                                <button onClick={()=> handleViewResume(app.resumeFile, app.userId)} className={s.resumeButton}>
                                                ...
                                                <span className={s.noResumeText}>No Resume Found</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
