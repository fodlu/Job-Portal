...
const Dashboard = () => {
    ...
    // to handle toast auto dismissal
    useEffect(() => {
    ...
    const handleConfirmClose = async() => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`http://localhost:500/api/job/${jobId}/close`, {
                method: "PATCH",
                headers: {
            ...
            if(data.success) {
                ...
                // refresh the stats
                const statsRes = await fetch(`http://localhost:500/api/job/admin/stats`, {
                    headers: {Authorization: `Bearer ${token}`}
                })
                ...
                const jobsRes = await fetch('http://localhost:500/api/job/admin/jobs', {
                    headers: {Authorization: `Bearer ${token}`}
                })
            ...
            console.log("Error closing the jobs", error);
            setToast({
                message: "failed to close Job", type: "error"
            })
        }
    }
    ...
    // fallback for thr logo that failed to load
    ...
    <div className={s.container}>
        {toast && (
            <div className={s.toastWrapper}>
                <div className={`${s.toastBase} ${toast.type === "success" ? s.toastSuccess : toast.type === "error" ? s.toastError : s.toastDefault}`}>
                    {toast.type === 'success' ? (
                    ...
                    <div className={s.toastFlex}>
                        ...
                        {toast.confirm && (
                            <div className={s.toastButtonContainer}>
                                <button onClick={handleConfirmClose} className={s.toastConfirmBtn}>Confirm</button>
                                <button onClick={()=> setToast(null)} className={s.toastCancelBtn}>Cancel</button>
                            </div>
                        )}
                    </div>
                    ...
                    return (
                        <div key={stat.label} className={s.statCard} >
                            ...
                            <div className={s.statCardContent}>
                                ...
                                <div className={`${s.statCardIconWrapper} ${stat.colors.bgLight} bg-linear-to-br ${stat.colors.gradient}`}>
            ...
            <div className={s.filtersContainer}>
                <div className={s.filtersHeader}>
                    ...
                    {(companyFilter || roleFilter || statusFilter !== "active"
                        ...
                        <button className={s.filtersClearBtn} onClick={clearFilter}>
                ...
                <div className={s.filtersGrid}>
                    <div className={s.filterInputContainer}>
                        <label className={s.filterLabel} htmlFor="filter">Filter By Company</label>
                        <div className={s.filterInputWrapper}>
                            <Search className={s.filterSearchIcon} />
                            <select value={companyFilter} onChange={(e)=> setCompanyFilter(e.target.value)} className={s.filterSelect}>
                    ...
                    {/* role filter */}
                    <div className={s.filterInputContainer}>
                        <label className={s.filterLabel} htmlFor="filter">Filter By Role</label>
                        <div className={s.filterInputWrapper}>
                            <Search className={s.filterSearchIcon} />
                            <select value={roleFilter} onChange={(e)=> setRoleFilter(e.target.value)} className={s.filterSelect}>
            ...
            <div className={s.jobsSection}>
                <div className={s.jobsHeader}>
                    ...
                    <div className={s.jobsFilterContainer}>
                        <select value={statusFilter} onChange={(e)=> setStatusFilter(e.target.value)} className={s.jobsStatusSelect}>
                ...
                {loading ? (
                    ...
                    <div className={s.jobsGrid}>
                        {filteredJobs.map((job) => (
                            <div className={s.jobCard} key={job.id}>
                    ...
                    <div className={s.emptyState}>
                        ...
                        <button onClick={clearFilter} className={s.emptyStateBtn}>
                            Clear Filters
                        </button>
                    </div>
                )}
